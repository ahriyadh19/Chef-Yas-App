...
class _PrinterServiceState extends State<PrinterService> {
  BluetoothPrint bluetoothPrinter = BluetoothPrint.instance;
  ...
  Future<void> initPrinter() async {
    bluetoothPrinter.startScan(timeout: const Duration(seconds: 2));
    if (!mounted) return;

    bluetoothPrinter.scanResults.listen(
      (event) {
        if (!mounted) return;
        setState(() {
          _devices = event;
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      decoration: BoxDecoration(
        color: const Color.fromARGB(255, 113, 67, 0).withOpacity(0.9),
        borderRadius: const BorderRadius.only(topRight: Radius.circular(25), topLeft: Radius.circular(25)),
      ),
      height: h / 2,
      child: _devices.isEmpty ? Text(_devicesMsg) : const Text('Test test'),
    );
  }
}
/**import 'dart:async';
